...
- `worker/client-drawlist.ts` (M17, steps 1-3 of docs/plan/17-drawlist-and-sprites.md): `createDrawlistPump`, called from `body()` right after a real `frame()` call (only when `CB_FRAME_REQ` advanced). `ABI_VERSION` 14 -> 15: `drawlist_len() -> u32` (client role, `RegionId.DrawList`'s own record count from the last `frame()`'s `sort_into`). Proportional publish (0018 §2, `sab/layout.ts`'s `DRAWLIST_HEADER_BYTES`/`DRAWLIST_BODY_BYTES`, now exported): header always copied, only the body blocks `drawlist_len() * 32` bytes actually span, via `sab/triple.ts`'s `BLOCK_BYTES` (also exported) and whole-block `copyBytes`. `engine/test`'s `drawListHash`/`drawListRecords` (`test/client.ts`) read the newest `drawList` slot directly off the SAB (no worker round trip, `netCounters`'s own precedent).
