...
- Ring records (M16): `actionRing` `[seq u32 LE][len u32 LE][UTF-8 JSON]`; `uiRing` `[kind u8][len u32 LE][JSON]` (kind `2` action result, `1` M16b's `Ui`), unknown kind skipped by `len`. `worker/client-drawlist.ts` (M17, `ABI_VERSION` 14 -> 15): `createDrawlistPump`, run from `body()` after `frame()`; `drawlist_len()` (records) drives a proportional copy (`sab/layout.ts`'s `DRAWLIST_HEADER_BYTES`/`DRAWLIST_BODY_BYTES`, `sab/triple.ts`'s `BLOCK_BYTES`, all exported) into `drawList`. `engine/test.drawListHash`/`drawListRecords` read the newest slot off the SAB directly.
