...
- **Milestone:** M05 in flight, base `6682458`, `docs/plan/05-codec-and-state-hash.md` (the re-bless of `fixtures/hash/golden/golden.json` in its step 5 is authorised by the orchestrator). M06 is ready after it.
- **State:** M01–M04 are done and on `main` (tag `harness-complete` at M04's `done` commit); `pnpm test && pnpm lint` is green (four suites: `rust`, `unit`, `wasm`, `browser`). Phase 3 runs as one orchestrating session with Sonnet implementers on `main` (ADR 0025). M04 landed the zero-allocation instrument (`tests/browser/gc/`, `zeroGcSuite`, page `gc-loop` with five permanent negative controls, tunnel and flat CDP transports), `packages/engine/budgets.json` + `tests/support/budgets.ts`, `installGcPage` in `engine/test`, `pnpm gc <software|flat|reliability>` and the `gc-test` skill; exact seam shapes are in each brief's Deviations. Known: commits `5cbf35e` (`M03 done`) and `e6767d6` are red on one `unit` test (a `CLAUDE.md` line cap, the orchestrator's slip; fixed in M04), so do not push those two as heads. Device check ready for Tyler: M03 determinism page (`docs/plan/device-checks.md`). Open questions for Tyler (none blocking; defaults stand): `docs/plan/questions-for-tyler.md`.
- **Exact next step:** On a new machine run `pnpm install && pnpm setup:tools` first. Start the session on `main` with edits auto-accepted, confirm `milestone-implementer` is among the agent types, then follow "The loop" from step 2 (in flight: `git log 6682458..HEAD` plus the brief's Deviations say how far M05 got). Run `pnpm test && pnpm lint` again after your own record edits, before every `done` commit. The next marker is M16 (`vertical-slice-complete`).
- **Blockers:** none.
