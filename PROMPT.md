...
- **Milestone:** M06b in flight, base `ac7a24d`, `docs/plan/06b-workers-and-spawn.md`. M07 is ready after it.
...
- **Exact next step:** On a new machine run `pnpm install && pnpm setup:tools` first. Start the session on `main` with edits auto-accepted, confirm `milestone-implementer` is among the agent types, then follow "The loop" from step 2 (in flight: `git log ac7a24d..HEAD` plus the brief's Deviations say how far M06b got). Check `uptime` before gating: the suite budgets are wall-clock, and at a load average near 20 every suite fails on timeouts only. When a milestone adds cross-thread or timing-sensitive tests, repeat its suite about 30 times under a per-run kill timeout before accepting (M06's hang showed once in 5–10 runs). Run `pnpm test && pnpm lint` again after your own record edits, before every `done` commit. The next marker is M16 (`vertical-slice-complete`).
- **Blockers:** none.
