...
- **Milestone:** M06b in flight, base `ac7a24d`, `docs/plan/06b-workers-and-spawn.md`: fully built, two fix rounds committed (last `37008df`), **not accepted**. What is verified, the acceptance checklist, three decisions owed and the records still missing are in `HANDOFF.md` (repo root, temporary). M07 is ready after it.
- **State:** M01–M06 are done and on `main` (tag `harness-complete` at M04's `done` commit); `pnpm test && pnpm lint` is green (four suites: `rust`, `unit`, `wasm`, `browser`). Phase 3 runs as one orchestrating session with Sonnet implementers on `main` (ADR 0025). M04 landed the zero-allocation instrument (`tests/browser/gc/`, `zeroGcSuite`, page `gc-loop` with five permanent negative controls, tunnel and flat CDP transports), `packages/engine/budgets.json` + `tests/support/budgets.ts`, `installGcPage` in `engine/test`, `pnpm gc <software|flat|reliability>` and the `gc-test` skill; M05 landed `engine::codec` (`Codec`, canonical NaN, `decode_canonical`), `engine::bytes` (`ByteSink`, `ByteReader`, the one varint), `StateHash` / `hash_value` / `mix64`, byte goldens (`assert_golden_bytes!`, `pnpm golden:bytes`) and a re-blessed `fixtures/hash` golden; M06 landed `src/sab/` (SPSC ring, seqlock, triple buffer, control block, `SabSet` / `createSabSet`, `RING_DEFAULTS`) and `src/camera/{state,block}.ts`, after two fix rounds for a livelock in `triple.test.ts`; `buildGame` now writes its outputs atomically; exact seam shapes are in each brief's Deviations. Known: commits `5cbf35e` (`M03 done`) and `e6767d6` are red on one `unit` test (a `CLAUDE.md` line cap, the orchestrator's slip; fixed in M04), so do not push those two as heads. Device check ready for Tyler: M03 determinism page (`docs/plan/device-checks.md`). Open questions for Tyler (none blocking; defaults stand): `docs/plan/questions-for-tyler.md`.
- **Exact next step:** On a new machine run `pnpm install && pnpm setup:tools` first. Start the session on `main` with edits auto-accepted, confirm `milestone-implementer` is among the agent types, read loop step 1, then **read `HANDOFF.md` and finish M06b from its section 2.2** (machine hygiene first: its section 3). The two Sonnet fix rounds are used, so a further failing gate goes to one implementer with `model: opus`. When M06b is recorded, delete `HANDOFF.md` in the `M06b done:` commit (fold its section 5 lessons that held up into Rules below) and continue at loop step 3 with M07. Check `uptime` before gating: the suite budgets are wall-clock. When a milestone adds cross-thread or timing-sensitive tests, repeat its suite about 30 times under a per-run kill timeout before accepting. Run `pnpm test && pnpm lint` again after your own record edits, before every `done` commit. The next marker is M16 (`vertical-slice-complete`).
- **Blockers:** none.
