...
- **Milestone:** M06 in flight, base `2be6625`, `docs/plan/06-sab-primitives-and-workers.md`: built (steps 1–6); **fix round 2 delegated** for an intermittent hang of the cross-thread `unit` tests (about 1 full `pnpm test unit` run in 5–10, on a quiet machine: one test thread spins at 100 % CPU forever; evidence and the required proof are in the delegation and go into the brief's "Open gate failures"). M07 is ready after it.
- **State:** M01–M05 are done and on `main` (tag `harness-complete` at M04's `done` commit); `pnpm test && pnpm lint` is green (four suites: `rust`, `unit`, `wasm`, `browser`). Phase 3 runs as one orchestrating session with Sonnet implementers on `main` (ADR 0025). M04 landed the zero-allocation instrument (`tests/browser/gc/`, `zeroGcSuite`, page `gc-loop` with five permanent negative controls, tunnel and flat CDP transports), `packages/engine/budgets.json` + `tests/support/budgets.ts`, `installGcPage` in `engine/test`, `pnpm gc <software|flat|reliability>` and the `gc-test` skill; M05 landed `engine::codec` (`Codec`, canonical NaN, `decode_canonical`), `engine::bytes` (`ByteSink`, `ByteReader`, the one varint), `StateHash` / `hash_value` / `mix64`, byte goldens (`assert_golden_bytes!`, `pnpm golden:bytes`) and a re-blessed `fixtures/hash` golden; exact seam shapes are in each brief's Deviations. Known: commits `5cbf35e` (`M03 done`) and `e6767d6` are red on one `unit` test (a `CLAUDE.md` line cap, the orchestrator's slip; fixed in M04), so do not push those two as heads. Device check ready for Tyler: M03 determinism page (`docs/plan/device-checks.md`). Open questions for Tyler (none blocking; defaults stand): `docs/plan/questions-for-tyler.md`.
- **Exact next step:** On a new machine run `pnpm install && pnpm setup:tools` first. Start the session on `main` with edits auto-accepted, confirm `milestone-implementer` is among the agent types, read loop step 1, then finish M06: if no `M06 fix 2` commit exists (`git log 2be6625..HEAD`), delegate fix round 2 again from the brief's "Open gate failures" (a third failed round goes to one implementer with `model: opus`); if it exists, accept on a quiet machine (`uptime` first): `pnpm gate 2be6625`, `pnpm test && pnpm lint`, and the full `unit` project at least 30 times in a row with a per-run kill timeout, 0 hangs. Then tick M06 (loop step 6) and continue at step 3 with M07. Run `pnpm test && pnpm lint` again after your own record edits, before every `done` commit. The next marker is M16 (`vertical-slice-complete`).
- **Blockers:** none in the code. M06's acceptance needs a machine quiet enough for the wall-clock suite budgets.
