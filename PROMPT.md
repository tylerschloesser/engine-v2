...
- **Milestone:** M06 in flight, base `2be6625`, `docs/plan/06-sab-primitives-and-workers.md`. M07 is ready after it.
...
- **Exact next step:** On a new machine run `pnpm install && pnpm setup:tools` first. Start the session on `main` with edits auto-accepted, confirm `milestone-implementer` is among the agent types, then follow "The loop" from step 2 (in flight: `git log 2be6625..HEAD` plus the brief's Deviations say how far M06 got). Run `pnpm test && pnpm lint` again after your own record edits, before every `done` commit. The next marker is M16 (`vertical-slice-complete`).
- **Blockers:** none.
