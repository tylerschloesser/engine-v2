...
- **State budget** (deterministic counts, part of world params, identical in every replay): `max_entities` default **262,144**, `max_modified_tiles` default **1,048,576**. A write that would exceed one returns `Err(Full)`: inside `apply` it becomes a normal rejection (`0004-action-timing-and-rejection.md`); inside a tick rule the game handles it (the machine stalls). It counts sim state, never allocator bytes or cache pressure.
