...
- **Logged movement action sampled from the camera** (research option b). The camera re-enters the sim through a side door; movement becomes 90%+ of the log (288-576 KB per player-hour quantized, against 14-58 KB); the spring becomes deterministic sim code; and predicting a springy body needs tick-stamped inputs, a server-side input jitter buffer and error smoothing, none of which discrete actions need.
