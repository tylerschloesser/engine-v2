...
fn tile_visual(t: Tile) -> TileTexel { TileTexel { base: t.base() as u16, resource: t.resource() as u16 } } // on chunk load/patch, never per frame
...
- `tile_visual` is an addition to the `ClientSide` trait listed in 0003. A game is limited to 256 tile images, 1,024 visuals and 4,096 sprites until an ADR adds a second array or atlas.
