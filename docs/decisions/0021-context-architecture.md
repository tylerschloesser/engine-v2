...
**6. One hook: a format-and-lint gate on `git commit`.** Created with `.claude/settings.json` in Phase 3's first milestone. A `PreToolUse` command hook, `matcher: "Bash"`, `if: "Bash(git commit *)"`, runs `.claude/hooks/pre-commit-check.sh`. The script reads the tool input from stdin, exits 0 immediately unless the command really is a `git commit`, then runs only format and lint checks that are deterministic and need no build: `cargo fmt --check` and `pnpm exec biome check .` (tools and pins: [0017](0017-packaging-and-build.md) section 10). On failure it exits 2 and prints the one command that fixes it (`cargo fmt` or `pnpm exec biome check --write .`). `timeout: 30`.
...
- Tests, type-checking (`tsc`), `cargo clippy` (it compiles) and builds are **not** in the hook. They are slow when caches are cold, browser suites can flake, and work-in-progress commits at the 50% context stop must stay possible. They are enforced by milestone exit criteria (`pnpm test` and `pnpm lint`, [0020](0020-testing-strategy.md)) and the `run-tests` skill.
