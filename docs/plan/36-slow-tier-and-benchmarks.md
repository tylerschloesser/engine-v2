...
- **WebKit readback** (`webkit-readback @slow`): one existing M09 readback scene with the same semantic probes in Playwright WebKit on macOS; records `adapter.info`; skipped by platform on Linux with a named notice (0020 §6 promises that adapter on macOS only). **A `@slow` tag alone does not scope a test to the `webkit` project** (M09 Deviations, "Gate round 3": the `chromium` project carries no project-level `grep`, so any `@slow`-tagged test — including M09's own pre-existing `terrain: probe tile colours webkit @webkit-gpu @slow` — already runs under `pnpm test:slow`'s `chromium` project too, headless-Chromium-executing a title that says "webkit"; flagged there as unfixed and left for whoever next touches that spec or the `chromium` project's grep). If `webkit-readback` must run only in real WebKit, guard it explicitly (`test.skip(browserName !== 'webkit')` or a project-scoped spec file), and do not assume the title tag is doing that scoping.
...
- **The number M36b's byte-diffing decision (0011) has to weigh against.** M15 measured `encode_chunk_snapshot`'s O(all entities)-twice-per-chunk cost under a realistic join — 2,000 entities across a 121-chunk view, one connection joining at once: **10.3 ms, 11,131 bytes**, alone exceeding 0010's entire 10 ms tick-CPU budget (`docs/plan/15-connection-and-subscriptions.md`'s Deviations, "Measured, not fixed"). M31 paces this across ticks; the busy furnace field measurement and the byte-diffing decision that depends on it are `36b`'s (Non-scope, above).
...
None of its own. It supplies `?bench=large-save` and the bench HUD for items M39-large-save and M39-frame-shares: [device-checks.md, M39](device-checks.md#m39-acceptance).
