...
## Non-scope
`extract`, DrawList, full `FrameView` (M17). `ClientSide::frame`, `FrameCx`, presence (M19). Overlay anchors (M18). Predicted clock and lead (M26). Engine events such as `Resyncing` on the TS surface (M37).
