...
## Exit criteria
- [x] All tests above pass; goldens regenerated by command.
- [x] `full_world_*` runs in under the 0020 §4 fast-tier p95 (use `testkit::fill_world` with small configured budgets, not the defaults).
...
- **`puts_script_a` re-blessed by the orchestrator** at `d5fd55ce8f13a67e` (`pnpm golden puts`; `puts_idle_100` and `puts-connected` unmoved): the script's `Bump`/`Remove` now find the entity `Spawn` created, as M12b's Deviations anticipated. The pinned value in `tests/wasm/puts.test.ts` and two doc comments were updated with it.
- **Review agent (~3,000-line diff), gate round 1:** the rewritten `entity_straddling_...` test anchored its wide entity in the *subscribed* chunk and so passed with anchor-only scopes (now anchored in the unsubscribed one, shown red against anchor-only `entity_scopes`); the Budgets row's `index_ops_per_put <= footprint area + 4` had no assertion (`Store::debug_index_ops` had no caller; now `index_ops_per_put_is_bounded_by_footprint_area_plus_four`); and nothing proved the game-side overlap guard (now `place_overlapping_an_existing_machine_by_one_tile_is_blocked`, in-chunk and across a border). The review confirmed each of the three anchor-to-footprint widenings has single-point coverage in `fixtures/machines/tests/loopback.rs`.
- **Accepted deviation, ledger row:** no debug panic on an overlapping put. With overlap, `ChunkIndex::entity_at` returns only the lowest id at that tile.
