...
Status: not started · After: 16 · Tyler-dependent: acceptance of `docs/decisions/0023-action-growth-declaration.md` (assumed accepted as drafted)
...
4. `docs/decisions/0023-action-growth-declaration.md` (or its numbered successor)
...
- **State-budget check** in `Host`/`Sim::step` before `apply`, per the draft ADR: `Game::growth`, `Growth`, saturating headrooms, audit after `apply`, counter `growth_violations`; plus the id-exhaustion clause of the entity-id ADR §2 in the same check. `WorldParams` budget fields become live; init check of the 0007 §8 memory split against the arena with real `size_of` (clean startup error, status code from `engine_init`).
...
## Non-scope
Timers, wake-ups, active lists, `TickCx` additions, undo journal (21b). Predicted placement and ghosts (M25, M33). Sprites for entities (M17). Overlay promotion to dense and bucketed area effects: not scheduled (0007 Consequences: no v1 rule needs them). Entity id layout: fixed by the entity-id ADR, built in M12.
...
## Seams
**Provides:** `ChunkIndex`, `Store::rebuild_indexes`, working `entity_at`/`traits_at`, footprint `Scopes`; `Game::growth`, `Growth`, `EngineReject::StateBudgetFull` raised for real, counter `growth_violations`, init status `BudgetExceedsArena`; fixture `machines`; goldens `machines/place-border`, `machines/full-world`; `testkit::fill_world(entities, tiles)` (bench-style genesis that M36's standard large save reuses).
**Consumes:** M12 `Store`, `Game::{prototype, anchor}`; M12b `Authority`, `Sim`; M15 frame builder and `Replica`; M14 `encode_chunk_snapshot`; M16 admit/apply pipeline and `onActionResult`; M07 `Registry::{add_prototype, prototype_traits, footprint, tile_traits}`, `Footprint`, `PrototypeId`, `TraitSet`.
...
- **Anchor hook:** uses `Game::anchor` as assumed in M12; if the ADR amendment chose `spawn(at, e)` instead, the change is confined to `Store::apply` and `Authority` scope derivation.
...
## Tests added
Rust: `footprint_sets_every_overlapped_chunk`, `despawn_clears_all_chunks`, `move_updates_old_and_new`, `entity_at_any_covered_tile`, `placement_is_one_trait_query` (water and another machine, same bit), `index_rebuild_equals_incremental`, `footprint_larger_than_chunk_panics_at_register`; loopback: `border_machine_delivered_once_to_partial_subscriber`, `border_machine_gone_when_last_overlapped_chunk_leaves`, `replica_traits_at_matches_host`; budget: `full_world_rejects_place_accepts_remove_then_place` (the second half the draft ADR adds to the 0003 scripted list), `undeclared_action_uses_max_action_growth`, `budget_verdict_replays_identically`, `growth_declarations_are_honest` (audit on, all `machines` scripts), `under_declared_growth_panics_in_debug`, `init_rejects_budget_over_arena`. Browser: none new (the slice test keeps passing).
