...
- **Own-timer completion gap** (`0012` Consequences, deferred 2→3): accepted as is. The bar fills on the predicted clock and the item appears up to one RTT later (one tick in single-player). Revisit in M34 with real latency; record the call in that brief's Deviations.
