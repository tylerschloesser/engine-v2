...
## Non-scope
- Entities, `EntityId`, entity stores, `ChunkIndex`, occupancy, the occupant term of `traits_at`, footprint-vs-chunk assertion, active lists, timer wheel, `TickCx`: M12 and M21. Entity store layout and id reuse are being decided in `docs/decisions/0022-entity-ids-and-provisional-ids.md`; nothing here depends on it.
- `WorldRead`/`WorldWrite`, `Unknown` outside a client subscription, deltas, `Store`: M12 (the replica decides what is `Unknown`; `TerrainStore` itself is total).
...
- `TilePos { x: i32, y: i32 }`, `ChunkCoord { x: i32, y: i32 }` (`key() -> u64`, `from_key`), `WorldPos { x: i32, y: i32 }` (Q24.8; `from_tile`, `tile()`, `clamped()`), `ChunkRect { min: ChunkCoord, max: ChunkCoord }` (inclusive; `expanded(rings)`, `contains`, `iter` row-major).
...
none (the 64 MiB split check is scheduled in M39; see Planning decisions 11).
