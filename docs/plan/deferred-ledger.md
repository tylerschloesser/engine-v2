...
| Determinism on real x86-64, physical iPhone, Android | **x86-64: closed.** Native (`scenario_matches_golden`), Node, Bun and Chromium all matched the Apple-silicon goldens on `ubuntu-latest` (run [35616810284](https://github.com/tylerschloesser/engine-v2/actions/runs/35616810284)); the Linux WebKit/Firefox leg (`@engines`, `pnpm test:slow`) closes the same run once M10 step 5 lands and is cited here then. The iPhone via the determinism page; Android not run: no device (Q5) | M10; M03 device check |
