...
- **`FrameView<'a, G>`** (this brief owns the final shape; it extends the struct M16b created, which already implements `WorldRead<G>` by delegating to `ClientCore::view()` and has `me()`, `authoritative_tick()`, `predicted_tick()`; 0003 describes it as "`WorldRead` + clocks + presences"). Added here:
  ...
  - `tick_fraction() -> f32` (progress into the current tick, for smooth progress drawables), `ticks_per_second() -> u32`, `time_ms() -> f64`
