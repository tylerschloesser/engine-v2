...
Split from PLAN row 22 (see M22). PLAN.md needs a row for this brief; M23 and M27 should list 22b, not 22, under After.
...
- Clean boundaries: M13's `SimHost.pause()` and `stop()` become async-complete: snapshot if dirty, then await `flush()` (0005 Cadence; 0013 World lifecycle: zero-player pause then idle snapshot. The 30 s idle timer and `onIdle` themselves are M27/M28).
