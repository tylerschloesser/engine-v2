...
- `import wasm from 'virtual:engine/wasm'` → `{ url: string, buildHash: string }` (type `EngineWasm`, declared in `engine/virtual`). This object is what `createClient({ wasm })` takes (M06) and what `engine/test` takes (M03).
