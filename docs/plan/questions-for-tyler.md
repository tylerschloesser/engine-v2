...
| Q8 | CI trigger policy and Actions minutes: run on every push to `main` (only `M<NN> done` commits are pushed, ADR 0025 §4) and every pull request, both tiers, superseded runs cancelled? | Yes | M10 | |
