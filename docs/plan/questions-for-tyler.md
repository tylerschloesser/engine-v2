...
| Q14 | **The one-line-edit rebuild misses your 30 s requirement** (`docs/spec/testing.md`): a comment added to `crates/engine/src/lib.rs` makes the next `pnpm test` build take ~150 s. Cargo compiles it in ~17.5 s; most of the rest is macOS code-signing/scanning of ~40 freshly linked test binaries (M17d Deviations, ADR 0033 §3). Will you allow your terminal (and whatever app runs Claude Code) as a "Developer Tool" (System Settings → Privacy & Security → Developer Tools; `spctl developer-mode enable-terminal` makes the pane appear), then re-time one edit? I believe that setting skips the per-binary scan, but I haven't verified it on this machine | No machine change. The requirement stays recorded as unmet at ~150 s. If the setting doesn't bring it under 30 s, I'll schedule consolidating `crates/engine`'s ~19 `tests/*.rs` files into fewer binaries myself (a technical call) | Nothing blocks on it; it's the edit-test loop's speed | |
