...
- **The allowlist has grown since (2026-09-19):** ADR 0025 §5 added `Bash(git tag -a *)`, so the `node -e` line under Verification commands, which pins the allow list to 0021 §7's, now exits 1. That is expected; the criterion was true when it was ticked, and the current list is owned by 0021 §7 as amended by 0025 §5.
