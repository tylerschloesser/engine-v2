...
## Scope
- Client role: call `ClientSide::frame(&mut FrameCx, &mut G::Presence)` once per `frame(t_ms)`, before `extract`; keep the value across frames; sample it into the uplink batch under the rate rule of 0010.
...
**Consumes:** `Presence`, `PresenceTable`, `FrameCx` shells (M12). The `frame(t_ms)` export and `FrameView` (M17; if nothing calls `ClientSide::frame` yet, this milestone adds the call with M12's `FrameCx` shell reading the camera block; M18 adds input events to it). `wire::UplinkBatch`/`UplinkWriter`/`UplinkReader` with the presence field and `SectionId` for Presence (M14; if the field was left out, add it and regenerate that golden). `Host::on_uplink`, `Host::build_frame`, `Host::disconnect`, `SubscriptionSet` membership per `ConnId`, `ClientCore`, `testkit::Loopback` (M15); `sim_admit` taking the whole batch (M15b). The `G::admit` call site, which passes `PresenceTable::empty()` until now, and the "not recorded" rule (M16). `Codec` (M05). World coordinate range check (M07).
