...
- Returning player: on its first `frame`, if the presence passed in was seeded (`seed_presence`, built by M28 — M19 scoped this method but did not implement it, per M19 Deviations — from `Welcome`) `RefClient` starts the spring there and calls `cx.follow(Some(pos))` for that one frame, then `None`. Otherwise M20b's spawn rule applies.
...
**Consumes:** `put_global` (M12b), `Delta::Roster` in the `Store` (M12; 0024 §8), Global section with the roster (M14); `FrameView::presences` with `alpha` (M19, M30); `seed_presence` from `Welcome` (M28; M19 scoped but did not build this method, per M19 Deviations); `cx.follow` (M18); `createClient` `host` option (M06b), `readInvite`, `wsUrl`, `client.onLink`, `startTestServer`, `games/reference-server`, `pnpm device:serve --app reference --ws` (M29); rates and hashes on by default (M31, M31b); `SimRng` through `WorldWrite::rng` (M12b).
