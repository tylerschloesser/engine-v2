...
6. **Tail re-execution runs on both paths**, exactly as 0005 says, with one guard: a record whose `G::Action` bytes do not decode under the new build is dropped with a `warn` and counted in the upgrade result; it is not an error. What happens to a tail whose bytes decode into a different valid action is 0024 §3's rule; implement it as written there.
