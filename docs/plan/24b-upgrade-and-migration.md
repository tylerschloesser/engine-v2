...
Split from PLAN row 24 (see M24). PLAN.md needs a row for this brief.
...
**Consumes**
- M12 `Game::{SCHEMA_VERSION, TICK_RATE, migrate}`, `WorldWrite`, `Authority`; M21 entity insertion path (occupancy, footprints), timer wheel and active lists; M07 `WORLDGEN_VERSION` + fingerprint; M22 `Identity`, `SnapshotReader`, `ManifestV1`; M22b `Persistence.open`, `WorldLoadError`, replay ABI, segment open/seal; M24 `SimHost.onRecovered` (fired with `reason: 'upgrade'`); M23 `SimLifecycleMessage` `start-failed`, `exportWorld`, `deleteWorld`; M06b `EngineStartError`.
...
6. **Tail re-execution runs on both paths**, exactly as 0005 says, with one guard: a record whose `G::Action` bytes do not decode under the new build is dropped with a `warn` and counted in the upgrade result; it is not an error. See the ADR note at the end.
7. **Order of writes makes the upgrade restartable:** new snapshot `write` → `flush()` → manifest `write` (old segment `sealed + tailReexecuted`, new segment entry) → new segment header `append`. A crash before the manifest write leaves the old world intact and the upgrade simply runs again; a stray new snapshot is pruned later. `SaveIncompatible` performs no write at all.
...
- Vitest (WASM under Node): `rules_only_change_direct_load_new_segment` (same `.wasm`, different `buildHash`: tail re-executed, old segment sealed + `tailReexecuted`, new segment based on the new snapshot, `onRecovered` fired with `'upgrade'`), `schema_bump_runs_migrate` (v1 world → v2 build), `tick_rate_change_without_bump_still_requires_migrate` (v2 → v2-hz30), `no_migrate_hook_save_incompatible_files_untouched` (v2 world → v1 build; storage byte-equal), `worldgen_stamp_mismatch_requires_migrate`, `undecodable_tail_action_is_dropped_and_counted`, `upgrade_crash_before_manifest_is_restartable` (`crashClone`), `import_then_upgrade` (archive from v1 imported under v2: 0005 "including the upgrade path").
...
ADR note for the planner (not an edit): 0005 Upgrades says re-executing the tail is safe because "at worst an action is now rejected". With postcard that is not strictly true when `G::Action`'s layout changed between builds: old bytes can decode into a different *valid* action. Decision 6 handles only the non-decoding case. Options for an amendment: drop the tail when `SCHEMA_VERSION` differs, or stamp an action-layout fingerprint in the segment header. In practice a clean shutdown snapshot leaves an empty tail.
