...
Hand-overs to read in place: M24 Non-scope (second bullet); M23 Seams (the TS paragraph); M29 "Link events"; M31b "Desync report". Rules that apply: `.claude/rules/hot-paths.md` (worker shells are touched; the healthy path must not change).
...
| `Resyncing` (`client.onResyncing`) | 0005 Panic recovery | M28b (host hook: M24) |
...
## Planning decisions
- **One delivery style: per-event subscriptions.** Engine events on the TS surface are `client.on<Name>(cb)` methods in the style of `client.onUi` / `client.onActionResult` (PRE-PLAN §4): `onStorage` (M23), `onResyncing` (M28b), `onLink` and `onVersionMismatch` (M29), `onRendererLost` (M37b), and this milestone's `onFatal` and `onDesync`. There is no `EngineEvent` union and no `client.onEngineEvent`; if the audit finds one in code, replace it. Start failures stay rejections of `client.ready` (M06b), never events.
...
- **ADR "Engine failure surface"** (next free number) records the four decisions above and the final event table, and links 0024 §5 for `HostServices.onFatal` instead of repeating it.
