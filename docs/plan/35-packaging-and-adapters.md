...
- [ ] `pnpm test:slow wasm -t size` and `-t "ts-rs zero bytes"` pass; `test-results/wasm/size.json` exists; a warn-level size is recorded in Deviations, not hidden.
...
- [ ] The ADR "Build profiles, measured" holds every number listed under Scope and the three profile decisions; root `Cargo.toml` matches it.
