...
- **`buildGame` must accept a crate path that goes through a symlink** (from M02b's Deviations: `artifactPath()` matches `cargo metadata`'s `manifest_path` by exact string, and macOS `$TMPDIR` is a symlink, so an un-realpath'd crate fails with the misleading "has no cdylib target"). Fix it in `build-game.ts` here (`realpath` the crate directory before matching; one `unit` or `wasm` test with a symlinked crate dir), since a game developer's project can sit under a symlink just as this scratch app does; then drop the `realpath()` workaround from `plugin-rebuild-error.test.ts`.
