...
### Open gate failures (orchestrator, gate round 3: the `browser` suite trip-wire)

Round 2 is accepted on evidence: mechanism shown by `--no-concurrent-recompilation` (25 of 25 clean) and the diff touches the instrument only; `node scripts/repeat.mjs browser 15 --load 10` at a 1-minute load of 23: pass 15, fail 0, hang 0. The orchestrator repeats the full 30 + 30 after this round. (Reservation recorded: `extraSettleFrames` waits by frame count for a wall-clock event. If `terrain clean` ever fails again with a fixed lump of about 22 KB on `client`, the next step is synchronous recompilation in the `gc` project's launch flags by an amendment to 0016, not a larger number.)

The trip-wire of `deferred-ledger.md` ("`browser` suite headroom") has tripped: quiet-machine `browser` is 24 s of 25 s with 80 tests, 31 s under `--load 10`. Measured split of test time (report.json, one loaded run, proportions): `gc` project 69 % (of which `burst` negatives on the production-topology pages `topology`, `echo`, `gen`, `terrain`: 36 % of the suite, 14 tests; `object` negatives 21 %; clean 6 %), `chromium` 22 %, `webkit` + `firefox` 9 %. Demoting the engine repeats alone leaves about 22 s, so both rungs are taken. Orchestrator's decision:

1. **WebKit and Firefox repeats move to the slow tier** (0020 §4, first rung; no ADR needed). `pnpm test browser` runs the `chromium` and `gc` projects only; `pnpm test:slow` (and M10's CI, which runs both tiers) runs every `@engines` test on WebKit and Firefox plus the `@webkit-gpu` scene, plus `@slow` titles as today. The mechanism is the implementer's (project selection in `scripts/lib/adapters.mjs` per tier is the obvious one; titles of existing tests do not change, and no existing test is deleted); `scripts/lib/adapters.test.mjs` fixes both command lines. `harness-complete`'s property "determinism hash equal in three browsers" is then a slow-tier and CI property: say so where `run-tests` and `packages/engine/CLAUDE.md` describe the tiers.
2. **`burst` negative controls of production-topology pages move to `@slow`; ADR 0026 records it** (`write-adr` skill; it amends 0016 §8 "Permanent negative controls" and cites 0020 §4; this section is the brief naming it). Decision text to draft from: page `gc-loop` keeps all five permanent negative controls in the fast tier, so both instruments are proven live on main and in a worker on every `pnpm test`; every page keeps its per-isolate `object` negatives in the fast tier, so instrument B is proven to see every isolate of every page; the per-isolate `burst` negatives of every other page (generated by `zeroGcSuite`) are tagged `@slow` and run in `pnpm test:slow` and CI. What the fast tier would lose is "instrument A looks at this page's thread X": keep it by having each page's **clean** test assert positively that the trace contains every expected isolate's thread (by the same pid / thread-name discovery A uses) with at least one event inside the marks, failing by name if a thread is missing; if the clean test already asserts that, cite the line in the ADR instead. 0020 §4's "never demote the only test covering a feature" is answered by that assertion, and the ADR says so. Alternatives to record as rejected: raising the suite budget; fewer frames per window; dropping per-isolate controls; more Playwright workers (the `gc` project's CDP tracing and wall-clock budgets are why it is 3; do not change `workers`).
