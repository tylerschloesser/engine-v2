...
**Pushing.** `git push` is outside the allowlist (0021 §7), so each push asks Tyler. Batch: get the workflow green in as few pushes as possible; use `gh run watch` and `gh run view --log-failed` to read results; download `test-results` with `gh run download` instead of re-running to look.
