...
- `region_hash`: M05 state hash over `encode_chunk_snapshot` of each held chunk in coord order + Global + OwnPlayer; `Host::region_hash(conn)` and `Replica::region_hash()`.
