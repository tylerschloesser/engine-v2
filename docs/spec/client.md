...
- **The camera is local; the sim's host only hears about it.** The camera responds instantly on the client and never mutates the world. It is reported to the sim's host as a non-action subscription message: how often, and with what lead (velocity-based look-ahead) so chunks arrive before they're visible. A game may still read the camera on the client (the reference game's player follows it; see `reference-game.md`).
